...
/** Headline facts from the summary, shown with the panel's existing status pills. */
function StatChips(summary: string): string {
    ...
    const chips = stats
        .map(
            (stat) =>
                `<span class="status-item stat-chip"><span class="stat-label">${escapeHtml(stat.label)}</span><span class="stat-value">${escapeHtml(stat.value)}</span></span>`,
        )
        .join('');
...
export function getResultsContent(
    ...
    const content = `<div class="shadow-wrapper"><div class="content-wrapper">${filterPanel}${StatChips(data.summary)}<div class="grid"><div>${summarySection}${buttonGroup}</div>${treeSection}</div>${contentSection}</div></div><script>${resultsScript()}</script>`;
    return createHtmlDocument(content, getBaseStyles(THEME));
}
